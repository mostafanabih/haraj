...
@section('content')
    <div class="container-fluid padding-r-l-50 " style="background: #F0F3F8;">
        ...
        <div class="col-sm-3 pull-left">
            <a href="{{ url('/cities/create') }}" type="button" class="btn btn-primary btn-block">
                    ...
                    @if(count($cities) > 0)
                        @for($i=0;$i<$cities->count();$i++)
                            <tr>
                                <td>{{ $i+1 }}</td>
                                <td>{{ $cities[$i]->name }}</td>
                                <td>{{ $cities[$i]->Area->name }}</td>
                                <td>
                                    <a style="margin-left: 25px" href="{{ url('/cities/'.$cities[$i]->id.'/edit') }}" class="btn btn-info pull-right"><i class="fa fa-pencil"></i>&nbsp;&nbsp; تعديل</a>
                                    <form onsubmit="return confirm('سوف يتم حذف جميع المعلنين والإعلانات المرتبطين بهذه المدينة ... هل تريد الحذف بالفعل ؟');" action="{{ url('/cities/'.$cities[$i]->id) }}" method="post">
                                        {{ method_field('DELETE') }}
                                        @csrf
